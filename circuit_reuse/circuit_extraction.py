...
class CircuitExtractor:
	def __init__(self, model: Any, top_k: Optional[int] = 5) -> None:
		if torch is None or HookedTransformer is None:
			raise ImportError("CircuitExtractor requires PyTorch + TransformerLens")
		self.model = model
		self.top_k = top_k
	...
	def extract_circuit_ig(self, prompt: str, target: str, baseline_prompt: Optional[str] = None, steps: int = 5) -> Set[Component]:  # pragma: no cover - heavier path
		...
		Interpolates embeddings between a baseline prompt (default: all zeros embedding of baseline tokens or an empty prompt) and the actual prompt, accumulating attribution for each component.
		"""
		if steps < 2:
